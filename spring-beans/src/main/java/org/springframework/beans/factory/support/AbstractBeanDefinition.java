...
public abstract class AbstractBeanDefinition extends BeanMetadataAttributeAccessor
	...
	/**
	 * 是否为抽象，对应bean属性 abstract
	 */
	private boolean abstractFlag = false;

	/**
	 * 是否延迟加载(懒加载)，对应bean属性lazy-init
	 */
	@Nullable
	private Boolean lazyInit;

	/**
	 * 自动注入模式，对应bean属性中的autowire
	 */
	...
	/**
	 * 用来表示一个bean的实例化依靠(依赖)另外一个bean先实例化，对应bean属性depend-on
	 */
	@Nullable
	private String[] dependsOn;

	/**
	 * autowire-candidate属性设置为false，这样容器再查找自动装配对象时，
	...
	/**
	 * 定义这个bean应用，APPLICATION：用户，INFRASTRUCTURE：完全内部使用，与用户无关，SUPPORT：
	 * 某些复杂配置的一部分
	 * 程序设置
	 */
